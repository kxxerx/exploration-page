# pollution-exploration standalone v1.17.14-souvenir-single-purchase-system-message

- SQL 실행 필요 없음.
- v1.17.9-personal-instant-items 기준 패치입니다.
- 라이브 룸 상단의 `방 설정` 버튼이 방장에게 다시 보이도록 복구했습니다.
- 사용자가 준 드림랜드 이미지를 어둑한 밤 톤으로 보정해 `assets/scenes/dreamland-night.webp`로 추가했고, 시나리오 상단 이미지로 고정했습니다.
- 시나리오 아이템 인벤토리를 탐사자별 개인 인벤토리 기준으로 표시하도록 수정했습니다. 다른 사람이 획득한 개인 아이템은 내 인벤토리에 보이지 않습니다.
- 조직별 기본 지급 아이템도 방 공유 인벤토리가 아니라 개인 인벤토리로 들어가도록 수정했습니다.
- 시스템 메시지에서 캐릭터명 뒤 `님`이 빠지지 않도록 통일했습니다. 예: `이강헌님이 다음과 같이 행동했습니다. : 쓰레기 수거함을 뒤진다.`
- GitHub에는 deploy-only ZIP의 압축을 푼 내용물만 main/root에 업로드하세요. ZIP 파일 자체를 올리면 안 됩니다. 압축 파일은 홈페이지가 아니라 인간의 압축 욕망 덩어리입니다.
- standalone ZIP은 백업/전체 확인용이고, 실제 배포는 deploy-only ZIP을 쓰면 됩니다.


## v1.17.14-souvenir-single-purchase-system-message
- 섹션 2에서 재난관리국 요원에게 `오방사계반을 사용한다` 선택지가 비활성 상태로 보이도록 추가했습니다.
- 인벤토리에서 오방사계반을 사용해 단서 상태가 활성화되면 기존 `직원용 출입구로 진입한다` 선택지가 표시/활성화됩니다.


## v1.17.14-souvenir-single-purchase-system-message
- 드림랜드 이미지를 시나리오 본문을 가리지 않는 상단 배너 방식으로 고정했습니다.
- 선택지 클릭 처리 중복/연타 방지 처리를 추가해 동전 획득 직후 다른 선택지가 먹통처럼 보이는 문제를 줄였습니다.


## v1.17.14 변경 사항
- 유리손포는 인벤토리 상세에서 사용 버튼이 뜨지 않도록 조정했습니다.
- 기념품샵 구매 선택지 4개는 방 전체 기준으로 한 번 구매 후 모두 비활성화되며, 비활성 선택지 자체는 계속 보이도록 수정했습니다.
- 시계탑 광장 이동 선택지는 구매 여부와 무관하게 계속 클릭할 수 있습니다.
- 주요 행동 시스템 메시지를 자연문 형태로 정리했고, 입장/퇴장 시스템 메시지 기록을 추가했습니다.


## v1.17.15-natural-system-message-coin-click-fix
- 낡고 이상한 동전 획득 시스템 메시지를 자연문으로 수정했습니다.
- 기존 “다음과 같이 행동했습니다” 형식의 시스템 메시지가 남아 있어도 렌더링 전 자연문으로 보정합니다.
- 즉시 획득 선택지 처리 후 팝업/로그 지연 때문에 다른 선택지가 먹통처럼 보이지 않도록 클릭 잠금 해제 순서를 조정했습니다.
- SQL 실행 필요 없음.
