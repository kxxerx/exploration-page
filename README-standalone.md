# v1.17.38-section-5-3-clocktower-test-guidance - VIP 진행 제안 메시지 핫픽스
...
- standalone에는 `migrations/upgrade-v1.17.38-section-5-3-clocktower-test-guidance.sql`이 포함되어 있습니다. deploy-only에는 migrations 폴더를 포함하지 않습니다.

# pollution-exploration v1.17.38-section-5-3-clocktower-test-guidance

## 적용 순서
1. `pollution-exploration-deploy-only-v1.17.38-section-5-3-clocktower-test-guidance
2. 압축을 푼 폴더 안의 내용물을 GitHub 저장소 `main` 브랜치 root에 업로드합니다.
   - ZIP 파일 자체를 올리면 안 됩니다.
3. Supabase SQL Editor에서 아래 SQL 파일 내용을 전체 실행합니다.
   - `migrations/upgrade-v1.17.38-section-5-3-clocktower-test-guidance
   - `migrations/upgrade-v1.17.38-section-5-3-clocktower-test-guidance
   - `migrations/upgrade-v1.17.38-section-5-3-clocktower-test-guidance
4. 이미 v1.17.38-section-5-3-clocktower-test-guidance
5. 브라우저에서 `Ctrl + F5`로 강제 새로고침합니다.

## v1.17.38-section-5-3-clocktower-test-guidance
...
- 상점 오염도 연계는 기존 v1.17.38-section-5-3-clocktower-test-guidance
...
- 가이드 2페이지 선택지 동의 안내 문구는 v1.17.38-section-5-3-clocktower-test-guidance
...
- 이번 버전에서 새 SQL은 없습니다. 단, v1.17.38-section-5-3-clocktower-test-guidance
...
## v1.17.38-section-5-3-clocktower-test-guidance 변경
- 돌발상황 수락/거절 버튼 오른쪽 정렬 보강
- 혼자 도망 시도/수락/거절 시스템 메시지 문구 수정
- deploy-only 배포 ZIP에서는 migrations 폴더 제외


## v1.17.38-section-5-3-clocktower-test-guidance 입장 메시지 중복 수정
...
- v1.17.38-section-5-3-clocktower-test-guidance: 파티찾기 모집 종료 배지가 카드 전체 폭으로 늘어나는 문제를 수정하고 제목/상태 배지를 한 줄 정렬로 고정
